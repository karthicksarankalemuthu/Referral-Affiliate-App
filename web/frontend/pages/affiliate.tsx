import { Layout,Modal,List,Page,Button,Card, IndexTable,useIndexResourceState,Grid,Icon,Link,LegacyStack} from "@shopify/polaris";
import {
  ArrowLeftMinor
...
export default function affiliate() {
  ...
  const getdata=async()=>{
    let res= await fetch("/api/get_affiliate_camp")
    let res1=await res.json()
       console.log(res)
      setdata(res1)
      setloading(false)
...
const ToDoList = ({items}:any) =>{
  ...
  const ToDoItem = ({todo}:any) =>{
    ...
    <td> <Button primary onClick={()=>{rule(todo.uuid)}}>Rule</Button></td>
    ...
    <td><Button destructive onClick={()=>{deletecamp(todo.uuid)}}>delete</Button></td>
...
function deletecamp(uuid:string){
  (async()=>{
...
<LegacyStack.Item>
    <Button primary onClick={()=>{navigate('/pricerulenew')}}>Add Campaign</Button>
